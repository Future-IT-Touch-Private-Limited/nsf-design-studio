...
export default function Navbar() {
  ...
  const [isServiceOpen, setIsServiceOpen] = useState(false); // service dropdown

  console.log(isServiceOpen,"service open check")
  ...
  return (
    <div
      className={`w-full md:fixed  ${
        fixed
          ? "shadow-2xl bg-white text-black"
          : " bg-white text-black lg:bg-transparent lg:text-white"
      }   top-0 z-50`}
    >
      <div className="flex justify-between items-center lg:gap-x-5 px-5 md:px-12 xl:px-32 py-3 md:py-4">
        {/* NSF Design Studio */}
        <div className="flex items-center space-x-2 pr-[150px]">
            <Link href="/">
        ...
        {/* Desktop Nav */}
        <nav className="hidden lg:text-xs xl:text-base lg:flex lg:text-nowrap space-x-4 lg:space-x-6 font-medium text-sm  relative">
          <Link href="/" className="hover:text-[#b08c54]">
            Home
          </Link>
          <Link href="/about-us" className="hover:text-[#b08c54]">About</Link>

          {/* Services with Hover Toggle */}
          {/* <div
            className="relative"
            onMouseEnter={() => setIsServiceOpen(true)}
            onMouseLeave={() => setIsServiceOpen(false)}
          >
            <button className="flex items-center space-x-1 hover:text-[#b08c54] focus:outline-none">
              <span>Services</span>
              <ChevronDown
            ...
            {isServiceOpen && (
              <div className="absolute left-0 top-full mt-2 w-44 bg-white shadow-md rounded-md z-50">
                <Link
                  href="#"
      ...
      >
        ...
        {isServiceOpen && (
          <ul className="absolute left-0 top-full   text-black bg-white shadow-lg rounded-md z-50">
            {service.map((elm,index)=>(
              <li key={index}>
              <Link
              href={`/service/${elm.title.toLowerCase().split(" ").join("-")}`}
              className="block px-4 py-2 text-sm hover:bg-gray-100"
            >
              {elm.title}
            </Link>
        ...
        <div className="flex items-center space-x-3">
          <div className="hidden lg:text-xs xl:text-base lg:flex items-center space-x-4">
            <button className="group flex items-center space-x-2 px-4 py-2 bg-[#7C877F] border border-gray-300 rounded-full shadow hover:bg-[#7C877F] hover:text-white transition text-sm">
              <span className="text-white">Book Consult</span>
              <span className="bg-white text-black rounded-full w-8 h-8 flex items-center justify-center text-xs transform transition-transform duration-300 group-hover:translate-x-1">
                <FaArrowRightLong />
              </span>
            </button>
          </div>
          ...
          >
            {isOpen ? (
              <X className="w-6 h-6 text-[#7c623f]" />
            ) : (
              <Menu className="w-6 h-6 text-[#7c623f]" />
            )}
          </button>
        </div>
      </div>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="lg:hidden px-5 md:px-12 xl:px-32 pb-4 transition-all duration-300 ease-in-out border-b border-gray-200">
          <div className="flex flex-col space-y-3 text-base text-gray-700 font-medium text-start">
            <Link href="/" onClick={()=>setIsOpen(false)} className="text-[#b08c54]">
              Home
            </Link>
            <Link href="/about-us" onClick={()=>setIsOpen(false)} className="hover:text-[#b08c54]">About</Link>
            ...
            <div>
              ...
              {isServiceOpen && (
                <ul className="mt-2 ml-3 flex flex-col space-y-2 text-gray-600 text-sm">
                 {service.map((elm,index)=>(
              <li key={index}>
              <Link
              href={`/service/${elm.title.toLowerCase().split(" ").join("-")}`}
